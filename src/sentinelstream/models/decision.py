...
def make_decision(signals: PredictionSignals, policy: ThresholdPolicy) -> RiskDecision:
    ...
    review_threshold = policy.threshold_for(signals.segment_key, signals.segment_observations)
    confidence = (
        signals.confidence if signals.confidence is not None else 2.0 * abs(probability - 0.5)
    )
