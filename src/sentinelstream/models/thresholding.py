...
def expected_financial_cost(
    ...
    *,
    ...
    missed_amount = float(transaction_amounts[(intervention_values == 0) & (y_true == 1)].sum())
    effective_missed_cost = (
        missed_amount * costs.false_negative_cost_per_amount * (1.0 - costs.recovered_funds_rate)
    )
