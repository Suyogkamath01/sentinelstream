...
from sentinelstream.streaming.topics import topic_definitions
...
def _streaming_topology(client: DashboardClient, settings: AppSettings) -> None:
    """Render the Kafka topology as an inspectable operator view."""

    st.header("Kafka streaming")
    st.caption(
        "SentinelStream's event-driven fraud path. The catalogue below is the declared "
        "contract; live broker state is shown only when the runtime is available."
    )

    ingress, validation, decision, outcomes = st.columns(4)
    stages = (
        (ingress, "1. Ingress", "transactions.v1", "Serving events enter Kafka."),
        (
            validation,
            "2. Validation",
            "validated-transactions.v1",
            "Schema and poison-event checks.",
        ),
        (decision, "3. Decisioning", "predictions.v1", "Features, model, and policy produce risk."),
        (outcomes, "4. Outcomes", "alerts.v1", "Alerts, audit, feedback, and DLQ records."),
    )
    for column, title, topic, description in stages:
    ...
    st.markdown(
        "`transactions`  →  `validated-transactions`  →  `predictions`  →  "
        "`alerts` / `audit` / `dead-letter`"
    )

    definitions = topic_definitions(settings.kafka)
    topic_rows = [
        {
            "Topic": definition.name.value,
            "Partition key": definition.key_strategy,
            "Partitions": definition.partitions,
            "Replication": definition.replication_factor,
        }
        for definition in definitions
    ]
    st.subheader("Declared topic catalogue")
    ...
    summary_left.metric("Declared topics", len(definitions))
    summary_middle.metric("Partitions per topic", settings.kafka.topic_partitions)
    summary_right.metric("Replication factor", settings.kafka.replication_factor)

    st.subheader("Runtime access")
    st.caption(f"Configured broker target: `{settings.kafka.bootstrap_servers}`")
    health = _load(client, client.dependencies, {})
    components = health.get("components", []) if health else []
    kafka_health = next(
        (component for component in components if component.get("name") == "kafka"), None
    )
    if kafka_health:
        status = str(kafka_health.get("status", "unknown"))
        st.metric("Kafka dependency health", health_label(status), kafka_health.get("detail"))
    else:
        st.info(
            "Kafka health is not exposed by the current API configuration. "
            "Use Kafka UI for live topics and offsets once the broker is running."
        )

    link_left, link_middle, link_right = st.columns(3)
    link_left.link_button("Open Kafka UI", "http://127.0.0.1:8080", use_container_width=True)
    link_middle.link_button("Open Spark UI", "http://127.0.0.1:8081", use_container_width=True)
    link_right.link_button("Open Grafana", "http://127.0.0.1:3000", use_container_width=True)
...
def main() -> None:
    ...
    pages: dict[str, Callable[[], None]] = {
        ...
        "Kafka streaming": lambda: _streaming_topology(client, settings),
